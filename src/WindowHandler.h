...
private:
	...
	void mouse_callback(double xpos, double ypos){
		...
		if (this->viewMode)
		{
			if (InputController::camChoice > 0 && CameraMaster::getCameras().size() > 0 && InputController::camChoice <= CameraMaster::getCameras().size()) {
				CameraMaster::getCameras()[InputController::camChoice - 1]->ProcessMouseMovement(xoffset, yoffset);
			}
			else {
